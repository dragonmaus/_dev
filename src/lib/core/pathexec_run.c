...
static stralloc temp;
  ...
  void
pathexec_run(const char *file, const char *const *argv, const char *const *envp)
{
  const char *path;
  unsigned int split;
  int savederrno;
  ...
  if (file[str_find(file, '/')]) {
    execve(file, (const char *const *)argv, (const char *const *)envp);
    return;
  }
  ...
  for (;;) {
    ...
    execve(temp.s, (const char *const *)argv, (const char *const *)envp);
